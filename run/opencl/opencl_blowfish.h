...
#ifndef _OPENCL_BLOWFISH_H
#define _OPENCL_BLOWFISH_H
...
#define BF_ENCRYPT     1
#define BF_ROUNDS     16
#define BF_BLOCKSIZE   8          /* Blowfish uses 64 bit blocks */

typedef struct {
	uint P[BF_ROUNDS + 2];    /*!<  Blowfish round keys    */
	uint S[4][256];           /*!<  key dependent S-boxes  */
} blowfish_context;
...
inline void blowfish_enc(blowfish_context *ctx, uint *xl, uint *xr)
{
	...
	Xl = Xl ^ ctx->P[BF_ROUNDS];
	Xr = Xr ^ ctx->P[BF_ROUNDS + 1];
...
{
	int i;
	uchar temp[BF_BLOCKSIZE];

	if (mode == BF_DECRYPT) {
		while (length > 0) {
			memcpy_macro(temp, input, BF_BLOCKSIZE);
			blowfish_crypt(ctx, mode, temp, temp);

			for (i = 0; i < BF_BLOCKSIZE;i++)
				output[i] = temp[i] ^ iv[i];

			memcpy_macro(iv, input, BF_BLOCKSIZE);
		...
		while (length > 0) {
			...
			blowfish_crypt(ctx, mode, temp, temp);
			memcpy_macro(iv, temp, BF_BLOCKSIZE);
			memcpy_macro(iv, temp, BF_BLOCKSIZE);
                                 ...
                                 BF_DST_TYPE uchar *output)
{
	...
	if (mode == BF_DECRYPT) {
		while (length--) {
			if (n == 0)
				blowfish_crypt(ctx, BF_ENCRYPT, iv, iv);
                               ...
                               BF_DST_TYPE uchar *output)
{
	...
	while (length--) {
		if (n == 0) {
			blowfish_crypt(ctx, BF_ENCRYPT, nonce_counter, stream_block);

			for (i = BF_BLOCKSIZE; i > 0; i--)
				if (++nonce_counter[i - 1] != 0)
					break;
		}
		c = *input++;
		*output++ = (uchar)(c ^ stream_block[n]);
