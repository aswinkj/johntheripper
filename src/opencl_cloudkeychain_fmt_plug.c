/*
 * This software is Copyright (c) 2018 Dhiru Kholia, Copyright (c) 2012-2013
...
#define FORMAT_LABEL            "cloudkeychain-opencl"
...
#define FINAL_KERNEL_NAME       "cloudkeychain_final"
...
static cloudkeychain_salt_t *host_salt;
...
static void create_clobj(size_t kpc, struct fmt_main *self)
{
	...
	host_salt = mem_calloc(1, sizeof(cloudkeychain_salt_t));
...
#define CLKERNELARG(kernel, id, arg, msg)  \
	...
	mem_salt = CLCREATEBUFFER(CL_RO, sizeof(cloudkeychain_salt_t),
...
static void *get_salt(char *ciphertext)
{
	...
	// we append the count and EOM here, one time <- salt hack required by our opencl kernel
	memcpy(&cs.salt[cs.saltlen], "\x0\x0\x0\x1\x80", 5);
	cs.saltlen = cs.saltlen + 5; // we include the x80 byte in our saltlen, but the .cl kernel knows to reduce saltlen by 1
...
static void set_salt(void *salt)
{
	...
	host_salt->hmacdatalen = cur_salt->hmacdatalen;
	memcpy(host_salt->hmacdata, cur_salt->hmacdata, cur_salt->hmacdatalen);
	memcpy(host_salt->expectedhmac, cur_salt->expectedhmac, 16);

	HANDLE_CLERROR(clEnqueueWriteBuffer(queue[gpu_id], mem_salt, CL_FALSE,
				0, sizeof(cloudkeychain_salt_t), host_salt, 0, NULL, NULL),
				"Salt transfer");
...
struct fmt_main fmt_opencl_cloudkeychain = {
	{
		FORMAT_LABEL,
		FORMAT_NAME,
		ALGORITHM_NAME,
