...
static cl_mem buffer_hash_ids, buffer_bitmap_dupe, buffer_bitmaps, buffer_salt;
...
{
	HANDLE_CLERROR(clSetKernelArg(crypt_kernel, 0, sizeof(buffer_keys), &buffer_keys), "Error setting argument 1.");
	HANDLE_CLERROR(clSetKernelArg(crypt_kernel, 1, sizeof(buffer_idx), &buffer_idx), "Error setting argument 2.");
	HANDLE_CLERROR(clSetKernelArg(crypt_kernel, 2, sizeof(buffer_int_key_loc), &buffer_int_key_loc), "Error setting argument 3.");
}

static void set_kernel_args()
{
	HANDLE_CLERROR(clSetKernelArg(crypt_kernel, 3, sizeof(buffer_int_keys), &buffer_int_keys), "Error setting argument 4.");
	HANDLE_CLERROR(clSetKernelArg(crypt_kernel, 4, sizeof(buffer_salt), &buffer_salt), "Error setting argument 5.");
	HANDLE_CLERROR(clSetKernelArg(crypt_kernel, 5, sizeof(buffer_bitmaps), &buffer_bitmaps), "Error setting argument 6.");
	HANDLE_CLERROR(clSetKernelArg(crypt_kernel, 6, sizeof(buffer_offset_table), &buffer_offset_table), "Error setting argument 7.");
	HANDLE_CLERROR(clSetKernelArg(crypt_kernel, 7, sizeof(buffer_hash_table), &buffer_hash_table), "Error setting argument 8.");
	HANDLE_CLERROR(clSetKernelArg(crypt_kernel, 8, sizeof(buffer_return_hashes), &buffer_return_hashes), "Error setting argument 9.");
	HANDLE_CLERROR(clSetKernelArg(crypt_kernel, 9, sizeof(buffer_hash_ids), &buffer_hash_ids), "Error setting argument 10.");
	HANDLE_CLERROR(clSetKernelArg(crypt_kernel, 10, sizeof(buffer_bitmap_dupe), &buffer_bitmap_dupe), "Error setting argument 11.");
}
...
static void create_clobj_kpc(size_t kpc)
{
	...
	if (ret_code != CL_SUCCESS) {
		saved_plain = mem_alloc(BUFSIZE * kpc);
		if (saved_plain == NULL)
	...
	saved_idx = clEnqueueMapBuffer(queue[gpu_id], pinned_saved_idx, CL_TRUE, CL_MAP_READ | CL_MAP_WRITE, 0, sizeof(cl_uint) * kpc, 0, NULL, NULL, &ret_code);
	...
	saved_int_key_loc = clEnqueueMapBuffer(queue[gpu_id], pinned_int_key_loc, CL_TRUE, CL_MAP_READ | CL_MAP_WRITE, 0, sizeof(cl_uint) * kpc, 0, NULL, NULL, &ret_code);
...
static void create_clobj(void)
{
	...
	buffer_salt = clCreateBuffer(context[gpu_id], CL_MEM_READ_ONLY, SALT_SIZE, NULL, &ret_code);
...
static void init_kernel(unsigned int num_ld_hashes, char *bitmap_para)
{
	...
	sprintf(build_opts, "-D SL3 -D OFFSET_TABLE_SIZE=%u -D HASH_TABLE_SIZE=%u"
	...
	opencl_build_kernel("$JOHN/opencl/salted_sha_kernel.cl",
	                    gpu_id, build_opts, 0);
...
static void set_salt(void *salt) {
	...
	memcpy(applebug, salt, SALT_SIZE);
...
{
	unsigned int *t = get_binary(source);
...
static void auto_tune(struct db_main *db, long double kernel_run_ms)
{
	...
	char key[PLAINTEXT_LENGTH + 1] = "313375432167890";
