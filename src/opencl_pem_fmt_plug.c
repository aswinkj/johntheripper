...
static pem_out *output;
static pem_salt currentsalt;
static cl_mem mem_in, mem_dk, mem_salt, mem_state, mem_out;
static size_t key_buf_size;
static int new_keys;
static struct fmt_main *self;

static cl_kernel pbkdf2_init, pbkdf2_loop, pbkdf2_final, pem_final;
...
static const char * warn[] = {
	"P xfer: ",  ", init: ", ", loop: ", ", pbkdf2: ", ", pem: ", ", res xfer: "
};
...
static size_t get_task_max_work_group_size()
{
	...
	s = MIN(s, autotune_get_task_max_work_group_size(FALSE, 0, pem_final));
	return s;
}
...
static void create_clobj(size_t gws, struct fmt_main *self)
{
	...
	output = mem_alloc(sizeof(pem_out) * gws);
	...
	mem_salt = clCreateBuffer(context[gpu_id], CL_MEM_READ_ONLY, sizeof(pem_salt), NULL, &ret_code);
	...
	mem_out = clCreateBuffer(context[gpu_id], CL_MEM_WRITE_ONLY, sizeof(pem_out) * gws, NULL, &ret_code);
	...
	HANDLE_CLERROR(clSetKernelArg(pem_final, 0, sizeof(mem_salt), &mem_salt), "Error while setting mem_salt kernel argument");
	HANDLE_CLERROR(clSetKernelArg(pem_final, 1, sizeof(mem_dk), &mem_dk), "Error while setting mem_dk kernel argument");
	HANDLE_CLERROR(clSetKernelArg(pem_final, 2, sizeof(mem_out), &mem_out), "Error while setting mem_out kernel argument");
}
...
static void done(void)
{
	if (program[gpu_id]) {
		...
		HANDLE_CLERROR(clReleaseKernel(pem_final), "Release kernel");
...
static void reset(struct db_main *db)
{
	if (!program[gpu_id]) {
		...
		HANDLE_CLERROR(ret_code, "Error creating kernel");
		pem_final = clCreateKernel(program[gpu_id], "pem_final", &ret_code);
		HANDLE_CLERROR(ret_code, "Error creating kernel");
	}
...
static void set_salt(void *salt)
{
	...
	memcpy(currentsalt.pbkdf2.salt, cur_salt->salt, SALTLEN);
	memcpy(currentsalt.iv, cur_salt->iv, 16);
	memcpy(currentsalt.ciphertext, cur_salt->ciphertext,
	       cur_salt->ciphertext_length);
	currentsalt.pbkdf2.length = SALTLEN;
...
static int crypt_all(int *pcount, struct db_salt *salt)
{
	...
	BENCH_CLERROR(clEnqueueNDRangeKernel(queue[gpu_id], pem_final, 1, NULL, &scalar_gws, lws, 0, NULL, multi_profilingEvent[4]), "Run PEM kernel");

	// Read the result back
	BENCH_CLERROR(clEnqueueReadBuffer(queue[gpu_id], mem_out, CL_TRUE, 0, sizeof(pem_out) * scalar_gws, output, 0, NULL, multi_profilingEvent[5]), "Copy result back");
...
struct fmt_main fmt_opencl_pem = {
	{
		FORMAT_LABEL,
		FORMAT_NAME,
		ALGORITHM_NAME,
