...
static void create_mask_buffers()
{
	...
	HANDLE_CLERROR(clSetKernelArg(crypt_kernel, 6, sizeof(buffer_bitmap),
	                              (void *)&buffer_bitmap), "Error setting argument 6");
}
...
static void prepare_bit_array()
{
	...
	do {
		...
		do {
			...
			// Skip cracked.
			if (binary) {
				SPREAD_32(binary[0], binary[1], binary[2], binary[3],
					(bitmap_size - 1U), bit_mask_x, bit_mask_y)
#ifdef DEBUG
